...
DWORD WINAPI LogWindowThread(LPVOID lpParam)
{
	...
	if (!DirectoryExists("client_messages"))
	{
		// Attempt to create the client_logs folder
		if (_mkdir("client_messages") != 0)
		{
			// Handle error creating directory
			MessageBox(NULL, L"Error creating logs directory!", NULL, MB_OK | MB_ICONERROR);
			return 1;
		}
	}
	// Get current date
	...
	localtime_s(&timeinfo, &now);
	char buffer[80];
	strftime(buffer, sizeof(buffer), "client_messages-%d-%m-%Y.log", &timeinfo);
	std::string logFileName(buffer);

	// Open the log file in append mode
	logFile.open("client_messages\\"+ logFileName, std::ios::app);
	if (!logFile.is_open())
	{

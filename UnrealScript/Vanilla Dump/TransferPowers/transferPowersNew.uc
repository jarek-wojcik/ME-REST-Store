...
public function createPowerSaveInfoAndReload(Class<SFXCharacterClass> fromCharacterClass, SFXMPCharacterRecord kitSave, int replacementIndex, int originalIndex)
{
    ...
    originalPower = Outer.PowerManager.GetPowerByClass(SFXPawn_PlayerMP(Outer).PlayerClass.SquadScreenPowerOrder[originalIndex]);
    ...
    SFXPawn_PlayerMP(Outer).PlayerClass.SquadScreenPowerOrder.Remove(originalIndex, 1);
    SFXPawn_PlayerMP(Outer).PlayerClass.SquadScreenPowerOrder.InsertItem(originalIndex, NewPowerClass);
    //This will update the HUD and allow us to use the new power.
    SFXPawn_PlayerMP(Outer).AutoMapPC();
}
public function LoadPower(PowerSaveInfo PowerSave, int originalIndex, out Class<SFXPowerCustomActionBase> powerClass)
{
    ...
    if (SFXPawn_Player(Outer) != None)
    {
        ...
        if (powerClass != None)
        {
            Power = Self.AddPower(powerClass, originalIndex);
        }
    }
    if (Power != None)
    {

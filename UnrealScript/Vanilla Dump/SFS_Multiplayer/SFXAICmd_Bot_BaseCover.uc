Class SFXAICmd_Bot_BaseCover extends SFXAICommand_Base_Combat within SFXAI_Bot
    abstract;
...
public function NotifyTakeHit(Controller instigatedBy, Vector HitLocation, int Damage, Class<DamageType> DamageType, Vector Momentum)
{
    ...
    if (ClassIsChildOf(DamageType, Class'SFXDamageType_Weapon'))
    {
        ...
        {
            Outer.BeginCombatCommand(Class'SFXAICmd_Bot_ShotInCover');
        }
    }
}
...
Begin:
    ...
    if (Outer.FireTarget == None)
    {
        ...
        {
            Class'SFXAICmd_AcquireCover'.static.AcquireNewCover(Outer, Outer.AtCover_WeaponRange, Outer.PlayerPawn);
